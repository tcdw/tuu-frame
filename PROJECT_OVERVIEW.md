...
*   `GET /api/presets`: Retrieves the current list of preset objects. Each object includes `id`, `mainPath`, `order` (e.g., 'shuffle', 'normal'), and an optional `name`.
*   `POST /api/presets`: Adds a new preset. Expects `{"mainPath": "/your/folder/path", "order": "shuffle", "name": "Optional Name"}`. The `id` is generated by the server. `order` defaults to 'shuffle' and `name` is optional.
*   `DELETE /api/presets`: Deletes a preset by its ID. Expects `{"id": "preset-id-to-delete"}`.
